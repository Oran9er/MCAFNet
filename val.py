...
def main():
    ...
    model = miccai_msnet.__dict__[config['arch']]()
    ...
    val_dataset = Dataset(
        img_ids=val_img_ids,
        img_dir=os.path.join('inputs', 'lung', 'images'),
        mask_dir=os.path.join('inputs', 'lung', 'masks'),
    ...
    meaniou_avg_meter = AverageMeter()
    ...
    with torch.no_grad():
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            iou,dice,meaniou,spe, sen,acc,f1 = iou_score(output, target)
            iou_avg_meter.update(iou, input.size(0))
            dice_avg_meter.update(dice, input.size(0))
            meaniou_avg_meter.update(meaniou, input.size(0))
    ...
    print('Meaniou: %.4f' % meaniou_avg_meter.avg)
